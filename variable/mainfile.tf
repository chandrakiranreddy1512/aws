#here we created a VPC and subnet, in that subnet we created a Ec2 instance.
#here we used variables instead of directly mentioning region, ami_id and instance type.
...
#here we mentioned "aws_vpc.var_vpc.id" , it takes the newly created vpc (line number 32)
